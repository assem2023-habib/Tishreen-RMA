...
المدير الأكبر لديه صلاحيات كاملة على النظام ولا تقتصر رؤيته على فرع واحد.

### إحصائيات النظام العامة
*   **المسار:** `GET /api/v1/super-admin/stats`
*   **الوصف:** الحصول على أرقام إجمالية عن الطرود، الفروع، الموظفين، والمستخدمين في كامل النظام.

### إدارة الفروع
*   **المسار:** `GET /api/v1/super-admin/branches` (عرض الكل)
*   **المسار:** `POST /api/v1/super-admin/branches` (إنشاء فرع جديد)
*   **المعلمات (POST):** `branch_name`, `city_id`, `address`, `phone`.

### إدارة الموظفين
*   **المسار:** `GET /api/v1/super-admin/employees` (عرض كل الموظفين)
*   **المسار:** `POST /api/v1/super-admin/assign-employee` (تعيين مستخدم لموقع فرع)
*   **المعلمات (POST):** `user_id`, `branch_id`.

### عرض كافة طرود النظام
*   **المسار:** `GET /api/v1/super-admin/all-parcels`
*   **الوصف:** عرض كافة الطرود الموجودة في النظام مع إمكانية التنقل بين الصفحات (Pagination).
